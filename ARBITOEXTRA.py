...
Parancssor:
  python arb_bot_ops_hardened_plus.py --help
"""
...
try:
    from tenacity import retry, retry_if_exception_type, stop_after_attempt, wait_exponential_jitter  # type: ignore
except Exception:
...
DEDUPE_ODDS_PRECISION = int(os.getenv("DEDUPE_ODDS_PRECISION", "3"))  # alap: 3 tizedes
...
def _setup_logging(level: int = logging.INFO) -> logging.Logger:
    ...
    sh.setFormatter(fmt)
    try:
        fh = RotatingFileHandler(SERVICE_LOG, maxBytes=5 * 1024 * 1024, backupCount=3, encoding="utf-8")
        fh.setFormatter(fmt)
        handlers = [sh, fh]
    except Exception:
        handlers = [sh]
...
class AppConfig:
    ...
    min_edge_market: Dict[str, Decimal] = None
    min_bet_per_book: Dict[str, int] = None
    min_step_per_book: Dict[str, int] = None
...
def load_config(yaml_path: Optional[str] = None) -> AppConfig:
    ...
    return AppConfig(
        ...
        regions=regions_val,
        time_window=int(os.getenv("TIME_WINDOW_HOURS", data.get("time_window", 48))),
        ...
        stake_round_step=int(os.getenv("STAKE_ROUND", data.get("stake_round_step", 1))),
        stake_round_profile=str(os.getenv("STAKE_ROUND_PROFILE", data.get("stake_round_profile", "flat"))).lower(),
        min_ttk_minutes=int(os.getenv("MIN_TTK_MINUTES", data.get("min_ttk_minutes", 120))),
        dedupe_window_sec=int(os.getenv("DEDUPE_WINDOW_SEC", data.get("dedupe_window_sec", 600))),
        ...
        allow_outrights=bool(int(os.getenv("ALLOW_OUTRIGHTS", "0"))),
        ...
        calib_min_edge_bounds=(D(str((data.get("calibration", {}) or {}).get("min_edge_bounds", [0.004, 0.012])[0])),
...
def mad_filter(offers: List[BookOffer], thr: Decimal = D("2.5")) -> List[BookOffer]:
    xs = [o.odds for o in offers if valid_price(o.odds)]
    if len(xs) < 3:
        return offers
...
def combined_filter(offers: List[BookOffer]) -> List[BookOffer]:
    return mad_filter(iqr_filter(offers))
...
def staleness_gate(offers: List[BookOffer], event_id: str, market_key: str, now_ts: int, thr: Decimal = D("0.75")) -> List[BookOffer]:
    kept: List[BookOffer] = []
    for o in offers:
...
def allocate_stakes_mv(fair_probs: Dict[str, Decimal], odds_map: Dict[str, Decimal], caps: Dict[str, Decimal], bank: Decimal, risk_aversion: Decimal) -> StakePlan:
    oH, oA = odds_map["home"], odds_map["away"]
...
class ConcurrencyGuard:
    """Adaptív, biztonságosabb módosítás: nem cseréljük le a Semaphore-t, csak növeljük release-szel."""
    def __init__(self, initial: int = 4):
        self._initial = max(1, initial)
        self._sem = asyncio.Semaphore(self._initial)
        self._current = self._initial  # logikai célérték
    ...
    def adapt_from_headers(self, headers: Dict[str, Any]) -> None:
        try:
            ...
            if r <= 3:
                ...
                new = self._initial
            new = max(1, min(self._initial, new))
            if new > self._current:
                for _ in range(new - self._current):
                    try:
                        self._sem.release()
                    except Exception:
                        pass
...
_GUARD = ConcurrencyGuard(int(os.getenv("ODDS_CONCURRENCY", "4")))
...
class ProviderBase:
    ...
    async def fetch_odds_for_sport(self, cfg: AppConfig, sport_key: str, commence_from_iso: Optional[str], commence_to_iso: Optional[str], markets: str) -> List[Dict[str, Any]]:
        raise NotImplementedError
    async def fetch_inplay_for_sport(self, cfg: AppConfig, sport_key: str, markets: str) -> List[Dict[str, Any]]:
        raise NotImplementedError
    async def fetch_event_odds(self, cfg: AppConfig, sport_key: str, event_id: str, markets: str, bookmakers: Optional[str] = None) -> Optional[Dict[str, Any]]:
        raise NotImplementedError
...
def _default_mock_event(sport_key: str, now_ts: int, inplay: bool = False) -> Dict[str, Any]:
    ...
    def mk(last_update_shift_sec: int = 0) -> str:
        return (datetime.now(timezone.utc) - timedelta(seconds=last_update_shift_sec)).isoformat().replace("+00:00", "Z")
    return {
        ...
        "bookmakers": [
            {
                ...
                "markets": [
                    ...
                    {"key": "totals", "outcomes": [{"name": "Over", "price": 2.05, "point": 2.5}, {"name": "Under", "price": 1.85, "point": 2.5}]},
                    {"key": "spreads", "outcomes": [{"name": "Alpha", "price": 1.90, "point": -1.5}, {"name": "Beta", "price": 2.00, "point": 1.5}]},
                    {"key": "draw_no_bet", "outcomes": [{"name": "Alpha", "price": 1.95}, {"name": "Beta", "price": 1.95}]},
                    {"key": "both_teams_to_score", "outcomes": [{"name": "Yes", "price": 1.95}, {"name": "No", "price": 1.95}]},
                ],
            },
            {
                "title": "William Hill",
                ...
                "markets": [
                    ...
                    {"key": "totals", "outcomes": [{"name": "Over", "price": 1.85, "point": 2.5}, {"name": "Under", "price": 2.05, "point": 2.5}]},
                    {"key": "spreads", "outcomes": [{"name": "Alpha", "price": 2.02, "point": -1.5}, {"name": "Beta", "price": 1.88, "point": 1.5}]},
                    {"key": "draw_no_bet", "outcomes": [{"name": "Alpha", "price": 2.05}, {"name": "Beta", "price": 1.85}]},
                    {"key": "both_teams_to_score", "outcomes": [{"name": "Yes", "price": 2.02}, {"name": "No", "price": 1.85}]},
                ],
            },
        ],
    }
...
class TheOddsAPIProvider(ProviderBase):
    ...
    async def fetch_sports(self, cfg: AppConfig) -> List[Dict[str, Any]]:
        if httpx is None or cfg.mode == "MOCK":
    ...
    async def fetch_odds_for_sport(self, cfg: AppConfig, sport_key: str, commence_from_iso: Optional[str], commence_to_iso: Optional[str], markets: str) -> List[Dict[str, Any]]:
        ...
        if cfg.mode.upper() == "LIVE" and httpx is None:
        ...
        if not key:
            return [_default_mock_event(sport_key, _now(), inplay=False)]
        params: Dict[str, Any] = {"apiKey": key, "markets": markets, "oddsFormat": "decimal", "dateFormat": "iso"}
    ...
    async def fetch_event_odds(self, cfg: AppConfig, sport_key: str, event_id: str, markets: str, bookmakers: Optional[str] = None) -> Optional[Dict[str, Any]]:
        if httpx is None or cfg.mode == "MOCK":
            return None
        client = await _get_client()
        params: Dict[str, Any] = {"apiKey": os.getenv("ODDS_API_KEY", ""), "markets": markets, "oddsFormat": "decimal", "dateFormat": "iso"}
        if bookmakers:
...
_OUTRIGHT_KEYWORDS = (
    "winner", "outright", "outrights", "special", "specials", "award",
    "mvp", "cy_young", "top_scorer", "golden_boot", "to_win",
    "championship_winner", "manager", "next_manager",
)
...
def apply_stake_rounding2(stakeH: int, stakeA: int, stepH_base: int, stepA_base: int, oa: Decimal, ob: Decimal, fx: Decimal, is_dnb: bool = False, profile: str = "flat") -> Tuple[int, int]:
    sH = int(stakeH)
    sA = int(stakeA)
...
def _compute_score(edge_after: Decimal, odds_age_sec: int, book_count: int, is_dnb: bool, dnb_factor: Decimal, inplay: bool, line_density: int) -> Decimal:
...
_CSV_HEADER = [
    "ts","latency_ms","sport_count","offer_count","event_id","sport_key","market","line",
    "book_a","book_b","odds_a","odds_b","inv_sum","edge_before_costs","edge_after_costs",
    "stake_a","stake_b","worst","cvar5","method","commence_time",
]

def _gzip_rotate(path: Path) -> None:
    try:
        import gzip, shutil  # noqa
        if not path.exists():
            return
        gz = Path(str(path) + ".gz")
...
def append_jsonl(row: Dict[str, Any], max_bytes: Optional[int] = None) -> None:
    try:
...
def append_csv(row: Dict[str, Any], max_bytes: Optional[int] = None) -> None:
    try:
...
def _save_dedupe(d: Dict[str, int]) -> None:
    try:
        ...
        try:
            DEDUPF.write_text(json.dumps(d, ensure_ascii=False), encoding="utf-8")
        except Exception:
            pass
...
def should_remind(arb: ArbOpportunity, interval_sec: int) -> bool:
    if interval_sec <= 0:
        return False
    now = _now()
    # csak pre-match (in-playre NINCS reminder)
...
def format_telegram(arb: ArbOpportunity) -> str:
    ...
    lines = [
        f"📣 ARB — {arb.sport_key} | {arb.home_team} vs {arb.away_team} | {_pretty_market(arb.market)} | {grade}{xtra}",
        f"🕒 {uk_time}  ({ttk})",
        f"Edge(before/after): {_fmt_pct(arb.edge_before_costs)} → {_fmt_pct(arb.edge_after_costs)} | Score: {float(arb.score):.3f}",
        f"Prices: {prices}",
        f"⏳ odds age: {_fmt_age(odds_age_sec)}",
        f"Stakes: A({arb.offer_a.book})={arb.stake_a}  B({arb.offer_b.book})={arb.stake_b}  | Worst: {float(arb.worst_case_profit):.2f} | CVaR5: {float(arb.cvar_5):.2f}",
        f"⏱ {arb.latency_ms} ms | offers={arb.offer_count} / sports={arb.sport_count} | id={arb.event_id}",
        "ℹ️ Info only. No auto-bets.",
    ]
    return "\n".join(lines)

def _http_post_json(url: str, data: Dict[str, Any], timeout: float = 10.0) -> Tuple[int, str]:
...
async def telegram_get_me(token: str) -> Tuple[int, str]:
    url = f"https://api.telegram.org/bot{token}/getMe"
    if httpx is None:
        return await asyncio.to_thread(_http_post_json, url, {})
    client = await _get_client()
...
def _trend_features_for_offer(event_id: str, market_key: str, offer: BookOffer, now_ts: int) -> Dict[str, Decimal]:
    ...
    age = D(min(1, max(0, now_ts - offer.ts) / 600.0))
...
class MLWrapper:
    ...
    @staticmethod
    def _feat_vector(inv_sum_f: float, age_sec: int, ewma_dev: float, cusum_mag: float, line_density: int, market_group: str) -> List[float]:
        # One-hot market_group (simple 5 dim)
    ...
    def predict01(self, inv_sum_dec: Decimal, age_sec: int, ewma_dev: Decimal, cusum_mag: Decimal, line_density: int, market_group: str) -> float:
        """0..1 score (magasabb jobb). Shadow módban csak logolni érdemes."""
        x = [self._feat_vector(float(inv_sum_dec), age_sec, float(ewma_dev), float(cusum_mag), line_density, market_group)]
...
# ───────────── discovery ─────────────
def _compute_trend_and_ml_boosts(cfg: AppConfig, ev: EventOdds, mk: str, a: BookOffer, b: BookOffer, line_density: int, inplay_evt: bool, inv_raw: Decimal) -> Tuple[Optional[Decimal], Optional[Decimal]]:
    ...
    if cfg.enable_ml and cfg.ml_weight > 0:
        ...
        oage = max(ageA, ageB)
        mg = _market_group(mk)
        if not hasattr(_compute_trend_and_ml_boosts, "_ml"):
            setattr(_compute_trend_and_ml_boosts, "_ml", MLWrapper(path=cfg.ml_model_path, shadow=cfg.ml_shadow_mode, weight=cfg.ml_weight))
        ml: MLWrapper = getattr(_compute_trend_and_ml_boosts, "_ml")
...
async def discover_arbs(cfg: AppConfig, provider: ProviderBase, stake_method: str = "robust") -> List[ArbOpportunity]:
    ...
    for ev in events:
        ...
        for mk in list(ev.offers.keys()):
            ...
            # NEW: trend + ML előszűrés, gate + súlyozás
            ...
            try:
                ...
                max_bytes = None
            append_jsonl(row, max_bytes=max_bytes)
            append_csv(row, max_bytes=max_bytes)

    if cfg.enable_middle_scan:
        for ev in events:
            total_lines = {k: v for k, v in ev.offers.items() if k.startswith("totals:")}
            try:
                lines_sorted = sorted(total_lines.keys(), key=lambda s: D(s.split(":", 1)[1]))
            except Exception:
                lines_sorted = sorted(total_lines.keys())
            for i in range(len(lines_sorted)):
                for j in range(i + 1, len(lines_sorted)):
                    ...
                    if not overL1 or not underL2:
                        continue
                    a = max(overL1, key=lambda x: x.odds)
                    b = max(underL2, key=lambda x: x.odds)
                    if a.book == b.book:
                        continue
                    logger.info(f"[EXPERIMENTAL MIDDLE] {ev.sport_key} {ev.home} vs {ev.away}: Over@{L1}({a.book} {a.odds}) / Under@{L2}({b.book} {b.odds})")
    return opps

# ───────────── prealert refetch ─────────────
async def _prealert_refetch_and_validate(cfg: AppConfig, provider: ProviderBase, opp: ArbOpportunity) -> bool:
    # FIX: fejlesztői/teszt (nem-LIVE) módokban ne dobjuk el az arbot a refetch miatt
    if cfg.mode.upper() != "LIVE":
        return True
    ...
    if opp.market == "h2h":
        ...
        markets = "h2h"
    refreshed = await provider.fetch_event_odds(cfg, opp.sport_key, opp.event_id, markets=markets, bookmakers=None)
    if not refreshed:
        return False
...
def _self_test_format_telegram():
    ...
    assert "ARB —" in txt and "Stakes:" in txt and "\n" in txt
...
def _calibrate(cfg: AppConfig) -> AppConfig:
    """Egyszerű, óvatos finomhangolás az utolsó nap alapján."""
    rows = _read_ledger_rows(days=max(1, cfg.calib_lookback_days))
    if not rows:
        logger.info("[CALIB] No ledger data for calibration.")
        return cfg
    n = len(rows)
    worsts: List[Decimal] = []
    edges_after: List[Decimal] = []
    stakes: List[int] = []
    books: Dict[str, int] = {}
    for r in rows:
        try:
            worsts.append(D(r.get("worst", "0")))
            edges_after.append(D(r.get("edge_after_costs", "0")))
            stakes.append(int(r.get("stake_a", 0)) + int(r.get("stake_b", 0)))
            books[r.get("book_a", "na")] = books.get(r.get("book_a", "na"), 0) + 1
            books[r.get("book_b", "nb")] = books.get(r.get("book_b", "nb"), 0) + 1
        except Exception:
            continue
    if not edges_after:
        return cfg
    med_edge = D(str(statistics.median(edges_after)))
    pnl_sum = sum(worsts) if worsts else D(0)
    stake_sum = sum(stakes) if stakes else 1
    pnl_perc = pnl_sum / D(stake_sum) if stake_sum else D(0)

    lo, hi = cfg.calib_min_edge_bounds
    new_min = cfg.min_edge_after_costs
    if pnl_perc < 0:
        new_min = min(hi, max(lo, cfg.min_edge_after_costs * D("1.10")))
    else:
        new_min = max(lo, min(hi, cfg.min_edge_after_costs * D("0.90")))
    ratio = (new_min / max(D("1e-12"), cfg.min_edge_after_costs))
    new_min_inplay = max(lo, min(hi + D("0.003"), cfg.min_edge_after_costs_inplay * ratio))

    sl_avg = dict(cfg.slippage_avg)
    sl_wc = dict(cfg.slippage_wc)
    if pnl_perc < 0:
        for k in list(sl_avg.keys()):
            sl_avg[k] = sl_avg[k] * D("1.10")
        for k in list(sl_wc.keys()):
            sl_wc[k] = sl_wc[k] * D("1.10")
    else:
        for k in list(sl_avg.keys()):
            sl_avg[k] = sl_avg[k] * (D("1.0") - cfg.calib_slippage_alpha)
        for k in list(sl_wc.keys()):
            sl_wc[k] = sl_wc[k] * (D("1.0") - cfg.calib_slippage_alpha)

    stake_profile = cfg.stake_round_profile
    if cfg.calib_drawdown_brake and pnl_perc < 0:
        stake_profile = "ladder"

    updated = dict(cfg.__dict__)
    updated["min_edge_after_costs"] = new_min
    updated["min_edge_after_costs_inplay"] = new_min_inplay
    updated["slippage_avg"] = sl_avg
    updated["slippage_wc"] = sl_wc
    updated["stake_round_profile"] = stake_profile
    logger.info(f"[CALIB] n={n} pnl_per_stake={float(pnl_perc):.4f} min_edge->{float(new_min):.4%} inplay->{float(new_min_inplay):.4%} profile={stake_profile}")
    return AppConfig(**updated)  # type: ignore

# ───────────── runner ─────────────
async def do_tg_test() -> int:
    token = os.getenv("TELEGRAM_TOKEN") or os.getenv("BOT_TOKEN")
    chat_id = os.getenv("TELEGRAM_CHAT_ID") or os.getenv("CHAT_ID")
    details = {
        "TELEGRAM_TOKEN": _mask(os.getenv("TELEGRAM_TOKEN")),
        "BOT_TOKEN": _mask(os.getenv("BOT_TOKEN")),
        "TELEGRAM_CHAT_ID": _mask(os.getenv("TELEGRAM_CHAT_ID")),
        "CHAT_ID": _mask(os.getenv("CHAT_ID")),
    }
    logger.info(f"Telegram env check (masked): {details}")
    if not token:
        logger.error("❌ Missing TELEGRAM_TOKEN (or BOT_TOKEN).")
        return 2
    if not chat_id:
        logger.warning("⚠️  CHAT_ID/TELEGRAM_CHAT_ID not set. Attempting getMe/getUpdates to help...")
        code, body = await telegram_get_me(token)
        logger.info(f"getMe -> {code} {body}")
        code, body = await telegram_get_updates(token)
        logger.info(f"getUpdates -> {code} {body}")
        logger.info("➡️  Send /start to your bot in Telegram, then set TELEGRAM_CHAT_ID to your user or chat id.")
        return 3
    text = f"✅ Bot connectivity test at {datetime.now(timezone.utc).isoformat()}"
    code, body = await telegram_send_with_retry(token, chat_id, text)
    logger.info(f"sendMessage -> {code}")
    if code == 200:
        logger.info("✅ Test message accepted by Telegram.")
        return 0
    else:
        logger.error("❌ Telegram rejected the request.")
        return 4

async def do_tg_debug() -> int:
    token = os.getenv("TELEGRAM_TOKEN") or os.getenv("BOT_TOKEN")
    chat_id = os.getenv("TELEGRAM_CHAT_ID") or os.getenv("CHAT_ID")
    logger.info(f"Telegram env (masked): {{'TELEGRAM_TOKEN': '{_mask(os.getenv('TELEGRAM_TOKEN'))}', 'CHAT_ID': '{_mask(os.getenv('CHAT_ID'))}'}}")
    if not token:
        logger.error("❌ Missing TELEGRAM_TOKEN (or BOT_TOKEN).")
        return 2
    code, body = await telegram_get_me(token)
    ...
    logger.info(f"getUpdates -> {code} {body}")
    if not chat_id:
        logger.info("ℹ️  No CHAT_ID set. If getUpdates shows messages, use the shown chat.id as TELEGRAM_CHAT_ID.")
    else:
        logger.info(f"CHAT_ID = {_mask(chat_id)}")
        code, body = await telegram_send_with_retry(token, chat_id, f"🔎 Debug ping {datetime.now(timezone.utc).isoformat()}")
        logger.info(f"sendMessage -> {code} {body[:160] if isinstance(body,str) else body}")
    return 0

async def _run(cfg: AppConfig, stake_method: str, notify: bool, tg_debug: bool = False) -> int:
    ...
    if not opps:
        logger.info("No opportunities this run.")
        return 0
    token = cfg.telegram_token or os.getenv("TELEGRAM_TOKEN") or os.getenv("BOT_TOKEN")
    chat_id = cfg.telegram_chat_id or os.getenv("TELEGRAM_CHAT_ID") or os.getenv("CHAT_ID")
    opps.sort(key=lambda o: o.score, reverse=True)
    for o in opps:
        ok = await _prealert_refetch_and_validate(cfg, provider, o)
        if not ok:
            msg = f"[DROP] Pre-alert refetch drop: {o.sport_key} {o.home_team} vs {o.away_team} | {_pretty_market(o.market)}"
            logger.info(msg)
            if tg_debug and notify and token and chat_id:
                await telegram_send_with_retry(token, chat_id, f"ℹ️ {msg}")
            continue

        txt = format_telegram(o)
        can_send = should_notify(o, cfg.dedupe_window_sec)

        sent_to_tg = False

        if can_send:
            # (1) ÚJ / odds-változott arb -> azonnali értesítés
            if notify and token and chat_id:
                code, body = await telegram_send_with_retry(token, chat_id, txt)
                logger.info(f"sendMessage -> {code}")
                sent_to_tg = (code == 200)
                if not sent_to_tg:
                    logger.error(f"Telegram error: {body[:200]}")
            if sent_to_tg:
                logger.info(txt)
            elif cfg.local_echo:
                logger.info("[LOCAL ONLY] " + txt)
        else:
            # (2) NINCS odds-változás -> csak pre-match emlékeztető (időzítve)
            if cfg.reminders_enabled and should_remind(o, cfg.reminder_interval_sec):
                remind_text = (
                    "🔔 Reminder — "
                    + f"{o.sport_key} | {o.home_team} vs {o.away_team} | {_pretty_market(o.market)}\n"
                    + f"🕒 {_uk_time_str(o.commence_time)}  ({_ttk_str(o.commence_time)})\n"
                    + "ℹ️ Odds nem változott az utolsó riasztás óta, de az arbitrage továbbra is érvényes."
                )
                if notify and token and chat_id:
                    code, body = await telegram_send_with_retry(token, chat_id, remind_text)
                    logger.info(f"sendMessage -> {code}")
                    sent_to_tg = (code == 200)
                    if not sent_to_tg:
                        logger.error(f"Telegram error: {body[:200]}")
                if sent_to_tg:
                    logger.info(remind_text)
                elif cfg.local_echo:
                    logger.info("[LOCAL ONLY] " + remind_text)
            # in-play esetén ide nem esünk be (should_remind False), tehát csend marad

    if notify and (not token or not chat_id):
        logger.warning("(!) --notify specified, but TELEGRAM_TOKEN / TELEGRAM_CHAT_ID missing — dry run.")
    return 0

def _print_missing_warnings() -> None:
    if not _missing:
        return
    sys.stderr.write(
        "[WARN] Optional dependencies are missing: " + ", ".join(sorted(set(_missing))) + "\n"
        "       The program will run with reduced features (e.g., slower HTTP without httpx,\n"
        "       no type validation without pydantic, no ML without scikit-learn, etc.).\n"
    )

# ───────────── service loop ─────────────
async def service_loop(cfg: AppConfig, args: argparse.Namespace) -> int:
    """24/7 loop. KeyboardInterrupt-ra kulturáltan leáll."""
    ...
    try:
        while True:
            ...
            cycle += 1
            purge_series(_now(), ttl=3600)
            errs = validate_config(cfg)
            if errs:
                logger.error(f"[CONFIG] Problems: {errs}")
                await asyncio.sleep(min(30.0, backoff))
                backoff = min(120.0, backoff * 2.0)
            else:
                try:
                    rc = await _run(cfg, stake_method=args.stake_method, notify=args.notify, tg_debug=args.tg_debug)
                    logger.info(f"[HEARTBEAT] cycle={cycle} rc={rc}")
                    backoff = 1.0
                except Exception as e:
                    logger.exception(f"[ERROR] run failed: {e}")
                    await asyncio.sleep(min(30.0, backoff))
                    backoff = min(120.0, backoff * 2.0)
...
def main() -> int:
    ...
    _GUARD = ConcurrencyGuard(int(os.getenv("ODDS_CONCURRENCY", "4")))
    if cfg.mode.upper() == "LIVE" and httpx is None:
        sys.stderr.write("[WARN] LIVE mode running without httpx — async HTTP is unavailable, fallback client used.\n")

    try:
        if args.tg_debug:
            rc = asyncio.run(do_tg_debug())
            return rc
        if args.tg_test:
            rc = asyncio.run(do_tg_test())
            return rc
        if args.once:
            rc = asyncio.run(_run(cfg, stake_method=args.stake_method, notify=args.notify, tg_debug=args.tg_debug))
            asyncio.run(_close_client())
            return rc
        # DEFAULT: 24/7 loop
        rc = asyncio.run(service_loop(cfg, args))
        return rc
